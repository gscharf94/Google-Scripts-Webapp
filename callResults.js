function callResultsStart(fileID, fileName) {
	...
	let timeInfo = createIndividualSheets(ssID, data);
...
function createIndividualSheets(ssID, data) {
	...
	for (const name in data) {
		let timeArr = createIndividualSheet(name, data[name], ss);
		timeInfo[name] = {};
...
function createOverviewSheet(ssID, data, timeInfo) {
	let ss = SpreadsheetApp.openById(ssID);
	...
	formatOverviewSheet(sheet, topRow.length, mainRangeValues.length, resultsTemplate, mainRange);
...
function formatOverviewSheet(sheet, width, height, template, mainRange) {
...
function addAverageRow(sheet, template, mainRangeValues) {
	...
	let rangeValues = [['Averages'],['Totals']];
...
function createIndividualSheet(callerID, personalData, ss) {
